Inception allows you to share behaviors recursively across types and their compositions using type-level programming instead of raw code generation.

For example, consider recreating our own version of the standard library Hash derive:

1. Define the behavior: the #[inception] macro assists building a proof of the behavior by structural induction.

```rust
#[inception(property = Digestible)]
pub trait Digest {
    // The signature of our behavior.
    fn digest(&self, state: &mut std::hash::DefaultHasher) -> ();

    // How should we handle the "end" of a list of types implementing our behavior
    fn nothing() -> () {}

    // How should we handle merging the outputs of our behavior across 2 types
    fn merge<H: Digest, R: Digest>(l: L, r: R, state: &mut std::hash::DefaultHasher) -> () {
        l.access().digest(state);
        r.digest(state);
    }

    // How should we handle merging the outputs of our behavior across enum variants
    ...
    // How should we handle a layer of structure (e.g a Struct or Enum)
...
2. Define some primitive types for the behavior: these are the minimal substructures of the composites, for which we have a custom implementation. For the sake of simplicity we can just delegate to the standard library behavior in these primitives.
...
3. Our behavior is now implemented for any Rust composite datatype that derives Inception, and has as its minimal substructures the primitives of our behavior.
...
fn main() {
    ...
    // Use the behavior that we defined
    let _ = game.digest(&mut std::hash::DefaultHasher::new()).finish();
}
```

The advantage of this pattern is that for a given type, the same amount of code is generated by the Inception derive macro regardless of the number of associated behaviors.

See the `inception-test` directory for examples of more advanced usage.

<details>
  <summary>Expand the more detailed original README that I wrote</summary>
